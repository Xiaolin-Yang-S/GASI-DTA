...
def train(architecture, predictor, device, train_loader, drug_graphs_DataLoader, target_graphs_DataLoader, LR, epoch, TRAIN_BATCH_SIZE):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        drug_embedding, target_embedding = architecture(drug_graph_batchs, target_graph_batchs)
    ...
    with torch.no_grad():
        for data in loader:
            drug_embedding, target_embedding = architecture(
                drug_graph_batchs, target_graph_batchs
            )
            output, _ = predictor(data.to(device), drug_embedding, target_embedding)
